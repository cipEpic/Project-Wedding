...
context("Header", () => {
    it("header content check", () => {
        cy.get(`[data-cy="header"] .content`).as("content").should("exist").children().should("have.length", 1);
    });

    it("Tech should be visible", () => {
        cy.get(`[data-cy="svg"]`).should("exist");
    });
});
...
context("Filter", () => {
    it("filter content check", () => {
        cy.get("[data-cy=filter] .contentf").as("contentf").should("exist").children().should("have.length", 1);
    });
});
